...
function extractDollarParens(command) {
  ...
  // so that:
  // 1. Parentheses inside quoted strings don't affect depth
  // 2. $() inside single-quotes is not extracted (shell treats as literal)
  // 3. $() inside double-quotes is extracted (shell DOES expand there)
  const candidates = [];
  let i = 0;
  let sq = false, dq = false, esc = false;
...
function isWorkflowCodexCall(cmd) {
  ...
  // Reject if command contains $() or backtick substitutions — the shell
  // executes these before Codex starts, so the command is not read-only.
...
function classifyAction(toolName, toolInput) {
  ...
  if (toolName === 'Bash') {
    ...
    // Non-rm destructive commands that need human confirmation
    if (isDestructiveNonRm(cmd)) {
      return { route: 'ask', reason: 'Destructive non-rm command, ask human', risk_category: 'destructive_shell' };
    }
