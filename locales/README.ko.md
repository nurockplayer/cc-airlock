...
`bypassPermissions` 를 활성화하면 Claude Code 는 모든 `ask` 권한을 자동으로 부여하여(`git push`, `npm install`, 파일 쓰기 등) 잠재적으로 위험한 명령을 실행할 때 프롬프트 없이 실행합니다.

이 플러그인은 안전 레이어를 복원합니다: **읽기 전용이 아니며 쓰기 전용도 아닌 모든 작업은 먼저 Codex(백업으로 DeepSeek)로 위험 평가를 전송**하고, **두 엔진 모두 명확히 `HUMAN` 을 반환할 때만** Claude 가 일시 중지하여 사용자 확인을 요청합니다. 그렇지 않은 경우 작업이 자동으로 계속됩니다.

간단히 말하면:
- ✅ 읽기 전용 도구 (Read, Grep, Glob, TaskList, WebFetch, …) → **즉시 통과**
- ✅ 쓰기 도구 (Write, Edit, MultiEdit) → **즉시 통과** (디자인에 따라)
...
설치 프로그램은 다음을 수행합니다:
1. 훅 스크립트를 `~/.claude/plugins/cc-airlock/hooks/` 로 복사
2. 아직 설정되지 않은 경우 `~/.claude/settings.json` 에 `"permissionMode": "bypassPermissions"` 추가를 요청
3. 모든 관련 툴 유형에 대해 Airlock 가드를 실행하는 `PreToolUse` 훅 추가

### 수동 설치 (원하는 경우)

1. `hooks/` 폴더를 `~/.claude/plugins/cc-airlock/` 로 복사
...
1. **내장 읽기 전용 도구** (`Read`, `Grep`, `Glob`, `TaskList`, …) → **즉시 통과**
2. **쓰기 도구** (`Write`, `Edit`, `MultiEdit`) → **즉시 통과** (디자인에 따라)
...
- 대부분의 일상 작업(파일 편집, 테스트 실행, 비강제 푸시 등)은 자동으로 승인됩니다.
