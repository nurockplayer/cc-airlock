...
```bash
git clone https://github.com/yourusername/cc-airlock.git
cd cc-airlock
...
The installer will:
1. Copy the hook scripts to `~/.claude/hooks/cc-airlock/`.
...
1. Copy the `hooks/` folder to `~/.claude/hooks/cc-airlock/`.
       ...
       "PreToolUse": [
         ...
         },
         {
           ...
           "hooks": [
             {
               "type": "command",
               "command": "node \"~/.claude/hooks/cc-airlock/codex-full-access-guard.js\"",
               "timeout": 30
             }
           ]
         }
...
1. Delete the folder `~/.claude/hooks/cc-airlock/`.
